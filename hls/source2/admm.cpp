...
// #pragma HLS UNROLL factor=2
	        tk_update(c_qp, RhoMt_neg, h_qp, R_inv, tk_admm, zk_admm, uk_admm);
	        //zk_update(c_qp, tk_admm, zk_admm, uk_admm);
	        //uk_update(c_qp, tk_admm, zk_admm, uk_admm);
	        zk_uk_update(c_qp, C_qp, tk_admm, zk_admm, uk_admm);
	    }
	return;
}
...
void uk_update(const data_t (&c_qp)[M_QP], const data_t (&C_qp)[M_QP][N_QP], const data_t (&tk)[N_QP], const data_t (&zk)[M_QP], data_t (&uk)[M_QP]){
	data_t temp[M_QP], temp1[M_QP], temp2[M_QP];
    {
//#pragma HLS LOOP_MERGE
    // uk = uk + (M*tk + zk - c);
